...
// TODO :   - define severals ENUMS
//          - implement multicast
//          - adapt names to uv functions
//          - add missing funtions (such as uv_udp_try_send)
...
duk_ret_t duv_udp_set_ttl(duk_context *ctx) {
  ...
  duv_check(ctx, uv_udp_set_ttl(udp, duk_get_int(ctx, 1)));
  return 0;
}
...
duk_ret_t duv_udp_broadcast(duk_context *ctx) {
  ...
  uv_udp_t *udp = duv_require_this_handle(ctx, DUV_UDP_MASK);
  duk_put_prop_string(ctx, 0, "broadcast");
